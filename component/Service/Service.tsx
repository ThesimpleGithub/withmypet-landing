...
const Service = ({ parentRef }: { parentRef: IRef }) => {
  ...
  useEffect(() => {
    // (MiddleRef.current! as HTMLElement).style.animation = 'unset';
    // setTimeout(() => {
    //   (MiddleRef.current! as HTMLElement).style.animation = 'fadeInOut .5s';
    // }, 100);
...
const LeftBtnGroup = ({ ...data }: IService) => {
  const changeScroll = () => {
    ...
    const scrollTo = top + window.scrollY - (innerWidth > 768 ? 80 : 60);
...
const MiddleCircle = ({ data, elem }: { data: IService; elem: any }) => {
  ...
  const scrollEvent = () => {
    ...
    if (Math.floor(window.scrollY) >= Math.floor(positionY)) {
      ...
      const diff = Math.abs((top - headerHeight) / totalHeight);
  ...
  useEffect(() => {
    const { top, height } = data.parent!.current!.getBoundingClientRect();
    const headerHeight = innerWidth > 768 ? 80 : 60;
    scrollObj.current = {
